...
% Initialize results table
results = table('Size', [6, 2], 'VariableTypes', {'double', 'double'}, ...
    'VariableNames', {'Setup', 'Parametric_pValue'});
...
for i = 1:6
    ...
    % Check for constant or insufficient data
    if length(preTMS_i) < 3
        warning('Setup %d has insufficient data. Skipping...', i);
        results.Parametric_pValue(i) = NaN;
        continue;
    end

    % Perform paired t-test
    [~, p_parametric] = ttest(preTMS_i, postTMS_i);
...
for i = 1:6
    ...
    fprintf('Parametric p-value: %.3f\n', results.Parametric_pValue(i));
    if isnan(results.Parametric_pValue(i))
        fprintf('Insufficient data for parametric test.\n');
    elseif results.Parametric_pValue(i) < 0.05
        fprintf('There is a significant difference between preTMS and postTMS based on the t-test.\n');
    else
        fprintf('There is no significant difference between preTMS and postTMS based on the t-test.\n');
    end
    fprintf('\n');
end
